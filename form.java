        ...
        button1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (inputText.length() % 2 == 0) {
                    outputText = inputText.substring(midIndex - 1, midIndex + 1);
                    for (int i = midIndex + 2; i < inputText.length(); i += 2) {
                        if (i + 1 < inputText.length()) {
                            outputText += inputText.substring(i, i+2);
                        } else {
                            outputText += inputText.substring(i);
                        }
                    }
                } else {
